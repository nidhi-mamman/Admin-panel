...
export default function CreateBranch() {
  ...
  const validateForm = (data) => {
    ...
    // branch required
    if (!data.branch) {
      fieldErrors.branch = "Please select a branch.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/admin/branches/create/", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        showToast(`Branch Created — username: ${formData.username}`, 'success');
        setMessage(data.message || "Branch created successfully ✅");
        navigate("/admin/show/branch-list");
      } else {
        console.error("Error:", data);
        const errMsg = data.message || "Error creating branch ❌";
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: "20px", marginLeft: "250px" }} className="create-staff-container">
      <div className="right-container">
        <h2 className="login-form-heading">Add Branch</h2>
        <form ref={formRef} onSubmit={handleSubmit} noValidate>
          ...
          <div className="form-group">
            <div style={{ marginTop: 8, width: "100%" }} className="form-group">
              <select name="branch" id="role">
                ...
                <option value="jalandhar1">Jalandhar1</option>
                <option value="jalandhar2">Jalandhar2</option>
                <option value="hoshiarpur">Hoshiarpur</option>
                <option value="ludhiana">Ludhiana</option>
                <option value="mohali">Mohali</option>
                <option value="phagwara">Phagwara</option>
                <option value="maqsudan">Maqsudan</option>
              </select>
          ...
          <div className="d-flex align-items-center justify-content-center">
            <button type="submit" className="custom-btn" style={{ padding: "8px 16px" }}>Add Branch</button>
          </div>
        </form>
